...
% complex index of reflection
   ...
   d(5)=tuning_wav/(4*index(3));        % AR coating
...
% interface matrices parallel polarization
    ...
    k=1; l=3 ; Ip13 = r_ip( N(k),N(l),phi(k),phi(l) ) ; % air/AR
	k=3; l=1 ; Ip31 = r_ip( N(k),N(l),phi(k),phi(l) ) ; % AR/air
...
% layer matrices
	...
	k=2 ; L2 = r_layer( N(2), d(k), phi(2), lambda );   % InP Bragg
 	...
 	k=5 ; L7 = r_layer( N(3), d(k), phi(2), lambda );   % AR coating
    
% scattering matrix

  Sp = Ip13*L7*Ip31*L3*Ip12*L2*(Ip21*L3*Ip12*L2)^(periodstop-1)*Ip21*L4*Ip12*(L2*Ip21*L3*Ip12)^(periodsbot-1)*L2*Ip21*L3*Ip12;	
...
%end  
   ...
   title('Air Gap Filter /w AR Coating')